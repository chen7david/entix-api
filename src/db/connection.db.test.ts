...
describe('Database Connection', () => {
  ...
  it('should update data successfully', async () => {
    ...
    // Update the record
    await pool.query('UPDATE test_users SET name = $1 WHERE id = $2', ['Robert Wilson', userId]);
  ...
  it('should handle concurrent transactions correctly', async () => {
    ...
    try {
      ...
      const transactionResult = await client.query('SELECT * FROM test_users WHERE email = $1', [
        'transaction@example.com',
      ]);
      ...
      const finalResult = await pool.query('SELECT * FROM test_users WHERE email = $1', [
        'transaction@example.com',
      ]);
