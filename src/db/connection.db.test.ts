...
describe("Database Connection", () => {
  ...
  it("should update data successfully", async () => {
    ...
    // Update the record
    await pool.query("UPDATE test_users SET name = $1 WHERE id = $2", [
      "Robert Wilson",
      userId,
    ]);

    // Verify the update
    const result = await pool.query("SELECT * FROM test_users WHERE id = $1", [
      userId,
    ]);
  ...
  it("should delete data successfully", async () => {
    ...
    // Verify data exists
    let result = await pool.query("SELECT * FROM test_users WHERE email = $1", [
      "alice@example.com",
    ]);
    expect(result.rows).toHaveLength(1);

    // Delete the record
    await pool.query("DELETE FROM test_users WHERE email = $1", [
      "alice@example.com",
    ]);

    // Verify deletion
    result = await pool.query("SELECT * FROM test_users WHERE email = $1", [
      "alice@example.com",
    ]);
